...
def main():
    ...
    p.add_argument("--no-flush-between-arms", dest="flush_between_arms", action="store_false",
                   help="Skip the extra cache flush before each of the 4 policy arms "
                        "within a trial. Faster (roughly halves HTTP round trips per "
                        "trial) but reopens a real, if usually small, cross-arm/cross-trial "
                        "contamination risk — see run_trial()'s docstring. Consider this "
                        "if you're timeboxed (e.g. a Kaggle session) and have already "
                        "confirmed via sanity_check_caching.py that caching is working "
                        "at all; leave it on otherwise.")
    p.add_argument("--no-track-real-hits", dest="track_real_hits", action="store_false",
                   help="Skip scraping vLLM's /metrics for real prefix-cache hit/query "
                        "counters around each trial (written to a companion "
                        "*_realmetrics.csv). Adds 2 lightweight GETs per trial; turn off "
                        "if that's not worth the wall-clock cost for your run.")
    args = p.parse_args()
    ...
    with VLLMLauncher(model_name=args.model, integration_path=args.integration_path) as launcher:
        ...
        try:
            ...
            csv_path = benchmark_harness.run_dataset(
                ...
                flush_between_arms=args.flush_between_arms,
                track_real_hits=args.track_real_hits,
            )
