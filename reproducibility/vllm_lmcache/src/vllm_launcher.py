...
class VLLMLauncher:
    ...
    def _build_vllm_mp_args(self) -> List[str]:
        # kv_connector_module_path: required from vLLM >=0.20.0 per LMCache's
        # own quickstart (docs.lmcache.ai) -- without it, "LMCacheMPConnector"
        # resolves to vLLM's OWN older, vendored, slower-moving connector of
        # the same name instead of LMCache's actual shipped one, on versions
        # new enough to have that ambiguity. Confirmed correct against
        # LMCache's current docs and a live GitHub issue thread using this
        # exact config.
        kv_transfer_config = (
        ...
        args = [
            ...
            # NOTE: --attention-backend CUSTOM removed. That flag is from
            # older LMCache/docs that predate vLLM's current attention-backend
            # registry: vllm/v1/attention/backends/registry.py defines
            # CUSTOM = None and raises "Backend CUSTOM must be registered
            # before use" (via register_backend()) if you pass the bare
            # string without registering a class first -- crashes at
            # model-init on any vLLM version new enough to enforce this
            # (confirmed against vLLM's own source). LMCache's connector
            # handles KV read/write independently of attention-backend
            # choice; leave it at vLLM's default (auto) instead.
    ...
    def start_inprocess(self):
        ...
        env = os.environ.copy()
        env.update(LMCACHE_ENV)
        # kv_connector_module_path + the "Dynamic" name suffix: this is
        # LMCache's documented "vLLM Dynamic Connector" pattern
        # (docs.lmcache.ai/api_reference/dynamic_connector.html), which
        # loads LMCache's own, independently-shipped connector class
        # instead of the older copy vendored inside vLLM itself under the
        # plain "LMCacheConnectorV1" name. Both name AND module_path have
        # to change together -- adding module_path while keeping the old
        # name (what I shipped in an earlier pass, copying an unverified
        # guess) doesn't point anywhere real. If this errors on your
        # installed LMCache version, the safe fallback is reverting to
        # plain '{"kv_connector":"LMCacheConnectorV1","kv_role":"kv_both"}'
        # (no module_path) -- confirmed valid, just resolves to vLLM's
        # bundled copy instead of LMCache's latest.
        kv_transfer_config = (
